...
void teensy_dummy_stall(void); // testing only, delete me before release
...
int main(int argc, char **argv)
{
	...
	// testing only, send a dummy request that will stall.
	// this allows us to make the first setup packet appear
	// on the opposite BDT (even vs odd) to make sure our
	// reception of data toggling works in both cases.
	...
	for (addr = 0; addr < code_size; addr += block_size) {
		...
		//for (r=0; r < write_size; r += 64) buf[r] = r / 64; // testing only!!!!!!
...
void teensy_dummy_stall(void)
{
	char buf[4096];
        ...
        usb_control_msg(libusb_teensy_handle, 0x21, 0x0A, 0x0000, 0, buf, 0, 4000);
}
